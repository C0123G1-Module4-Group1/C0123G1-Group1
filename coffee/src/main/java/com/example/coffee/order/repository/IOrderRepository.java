...
//    void setDeleteStatusToTrue(@Param("id") Integer id);
    ...
    @Query(value = "SELECT * FROM orders AS O WHERE O.code_order LIKE concat('%', :id, '%') AND status_delete = 0", nativeQuery = true)
    Page<Order> findAllByIdContainingAndDeleteStatusIsFalse(@Param("id") String id, PageRequest id1);
//
    @Query(value = "SELECT * FROM orders WHERE id = :id AND status_delete = 1", nativeQuery = true)
    Order findByIdOrder(@Param("id") String id);

//    Optional<Order> findCouponsByDeleteStatusIsFalseAndCodeCoupons(String result);

    Optional<Order> findCouponsByDeleteStatusIsFalseAndCodeOrder(String result);
}
